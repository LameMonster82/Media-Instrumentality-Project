...
export function TestSupportedPixelFormats() {
    const supported_pxl_formats = [AVPixelFormat.AV_PIX_FMT_NONE];
    function getEstimatedBufferSize(format: ExtendedVideoFormats, width: number, height: number) {
        ...
        switch (format) {
            ...
            case "I444A":
                return pixels * 4;
            default:
                return pixels * 4; // Conservative default
        }
    }
    ...
    extendedVideoFormatsArray.forEach(format => {
        try {
            ...
            supported_pxl_formats.unshift(VideoFormatToAVPixelFormat(format));
        } catch (e) {
            //console.log(format, "is NOT supported");
        }
    });

    return supported_pxl_formats;
}
...
export const supported_pxl_formats: AVPixelFormat[] = TestSupportedPixelFormats();
...
export function frameToGLEnumFormat(format: ExtendedVideoFormats, gl: WebGL2RenderingContext): { format: GLenum, type: GLenum; } {
    switch (format) {
        case "I420A":
        case "RGBA":
