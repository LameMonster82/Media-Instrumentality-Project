...
class ASS {
  ...
  constructor(content, video, { container = video.parentNode, resampling } = {}) {
    ...
    const { info, width, height, styles, dialogues, formats } = compile(content);
  ...
  /** @param {string} Subtitle to add. */
  addDialogue(dialogue) {
    var ref$1 = dialogue.match(/^(\w+?)\s*:\s*(.*)/i);
    var value$1 = ref$1[2];
    var dialogueParsed = parseDialogue(value$1, this.#store.formats);
    var compiledDialogues = compileDialogues({
      styles: this.#store.styles,
      dialogues: [dialogueParsed],
    });

    var dialogues = compiledDialogues.map((dia) => Object.assign(dia, {
      effect: ['banner', 'scroll up', 'scroll down'].includes(dia.effect?.name) ? dia.effect : null,
      align: {
        // 0: left, 1: center, 2: right
        h: (dia.alignment + 2) % 3,
    ...
    this.#store.dialogues.push(...dialogues);
    this.#store.dialogues.sort((a,b) => a.start - b.start)
  }

  /** @param {number} Time to seek. */
  customSeek(time) {
    clear(this.#store);
    const { video, dialogues } = this.#store;
    const vct = time - this.#store.delay;
    this.#store.index = (() => {
      for (let i = 0; i < dialogues.length; i += 1) {
        if (vct < dialogues[i].end) {
          return i;
    ...
    framing(this.#store, time);
  }
}

export { ASS as default, compile };
