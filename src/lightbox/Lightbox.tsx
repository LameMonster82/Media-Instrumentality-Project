...
import videoStyles from "@/player/controls/videoControls.module.css";
import { AssetType } from "@/assets/types";
import { formatBytes } from "@/library/types";
...
export class Lightbox {
    ...
    appendExifData(asset: Asset) {
        ...
        this.sideMenu.appendChild(
            <div class={ styles.titleBar }>
                <span onclick={ _ => this.toggleMenu() } class={ styles.side_button } style={ { marginLeft: "1rem" } }>arrow_back</span>
                <h1>{ asset.handle.name }</h1>
                <span onclick={ _ => this.closeDialog() } class={ styles.side_button }>close</span>
            </div>
        );
    ...
    async playMedia(asset: Asset, lightboxImage: HTMLImageElement) {
        ...
        const thumbnail = asset.thumbnailUrl && asset.thumbnailUrl.length > 0 ? asset.thumbnailUrl : lightboxImage.src;
