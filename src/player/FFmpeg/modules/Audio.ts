...
export function submitAudioConfig(config: { index: number, codec: string, sampleRate: number, numberOfChannels: number, description: number, descriptionSize: number; duration: number; })  {
    ...
    if (!('AudioDecoder' in self) || config.codec === '') {
        ...
        const areThereOtherStreams = workerState.isStreamTypeUsed("audio");
        // Safari time
    ...
    if (config.descriptionSize > 0)
        desc = workerState.readMemory(config.description, config.description + config.descriptionSize);
...
export function submitAudioFrame(data: { channels: number, sampleRate: number, samples: number, data: number, bytesPerSample: number; ts_js: number; stream_index: number; })  {
    const dataSize = data.channels * data.samples * data.bytesPerSample;
    const buffer = workerState.readMemory(data.data, data.data + dataSize);
